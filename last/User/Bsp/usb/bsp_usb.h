...
#define VISION_TX_FRAME_LEN   11
    ...
    /**
     * PC → MCU 下行帧（11字节）
     * pitch_rad：目标俯仰角，度，世界系绝对值
     * yaw_rad  ：目标偏航角，度，相对云台上电零点的增量
     */
    typedef struct __attribute__((packed)) {
        uint8_t header;         /* 0xA5                     */
        float   pitch_rad;      /* 目标 pitch（度，世界系）  */
        float   yaw_rad;        /* 目标 yaw  （度，相对零点）*/
    ...
    /**
     * MCU → PC 上行帧（11字节）
     * pitch_rad：当前俯仰角，度，IMU Mahony 解算
     * yaw_rad  ：当前偏航角，度，编码器差分累积
     */

    typedef struct __attribute__((packed)) {
        uint8_t header;         /* 0xA5                    */
        float   pitch_rad;      /* 当前 pitch（度，IMU）   */
        float   yaw_rad;        /* 当前 yaw  （度，编码器）*/
        uint8_t mode;           /* 当前工作模式            */
        uint8_t checksum;       /* XOR[1-9]             */
    } VisionTxFrame_t;
