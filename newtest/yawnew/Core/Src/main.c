...
/* ===== 归零参数 =====================================================
 *
 *  RTZ_PITCH_THRESH  Pitch 到位门限（°），IMU pitch 误差小于此值视为到位
 *  RTZ_YAW_THRESH    Yaw   到位门限（°），IMU yaw  误差小于此值视为到位
 *
 *  RTZ_PITCH_SPD_MAX 外环输出限幅（°/s），决定 Pitch 归零最大速度
 *  RTZ_YAW_SPD_MAX   外环输出限幅（°/s），决定 Yaw   归零最大速度
 *                    调小此值可以让归零过程更缓慢，推荐 20~50 °/s
 *
 *  两轴均以 IMU 输出为反馈，目标均为 0°：
 *    Pitch（MOTOR4）→ IMU pitch = 0°（真实水平）
 *    Yaw  （MOTOR2）→ IMU yaw   = 0°（上电校准时的正前方方向）
 *
 *  归零到位后重新记录编码器零点，使扫描段基准与归零位置对齐。
 * ================================================================== */
#define RTZ_PITCH_THRESH   2.0f
#define RTZ_YAW_THRESH     2.0f
#define RTZ_PITCH_SPD_MAX 30.0f   /* °/s，调小可更缓慢 */
#define RTZ_YAW_SPD_MAX   30.0f   /* °/s，调小可更缓慢 */
 ...
 *
 *  STATE_RTZ     上电归零：
 *                  Pitch（MOTOR4）用 IMU pitch 闭环到 0°（真实水平）
 *                  Yaw  （MOTOR2）用 IMU yaw   闭环到 0°（上电正前方）
 *                  外环输出限幅为 RTZ_*_SPD_MAX，保证缓慢运动
 *                  两轴均到位后清空 PID 积分，重记编码器零点，
 *                  切换到 STATE_HOMING
 *
 *  STATE_HOMING  归位：虚拟目标 prof_pos 缓慢收敛到 0，编码器跟随
 *
 *  STATE_SCAN    扫描：正弦轨迹
 *
 *  dbg_state：0=归零  1=归位  2=扫描
 * ================================================================== */
typedef enum {
  STATE_RTZ    = 0,
  STATE_HOMING = 1,
...
ctrl_state_t ctrl_state = STATE_RTZ;   /* 全局唯一状态，两轴共用 */
 ...
 *
 *  电机相关：
 *    dbg_angle2 / dbg_angle4      当前编码器角度（°）
 *    dbg_prof                     虚拟目标（扫描段）
 *    dbg_err2 / dbg_err4          外环误差
    ...
    if (HAL_GetTick() - last_tick >= 10)   /* 10ms = 100Hz，与 MahonyAHRS.c sampleFreq 一致 */
    {
      ...
      if (!imu_calib_done)
      {
        ...
        if (calib_cnt >= calib_total)
        {
           ...
           * Yaw=0 即为上电时的正前方方向，归零段 yaw_m=0 即回到此方向 */
          {
          ...
          imu_calib_done = 1;
          /* 校准结束后进入归零阶段，ctrl_state 已初始化为 STATE_RTZ */
      ...
      /* yaw_m 不做偏置补偿，校准时 Yaw 初始四元数已置为 0，
       * yaw_m = 0 即为上电时的正前方方向，直接用于归零控制 */
      dbg_yaw = yaw_m;

      /* ---- 3e_offset. Pitch/Roll 安装偏置补偿 -------------------------
       *
       *  上电校准结束时记录的 imu_pitch_offset / imu_roll_offset，
       *  即为云台在编码器零点时 IMU 的输出值。
       *  减掉后 IMU 零点与编码器零点自动对齐，无需手动填数。
       *  Yaw 不做此补偿，校准时四元数初始 Yaw=0 已是绝对零点。
       * ----------------------------------------------------------------- */
      ...
      dbg_state = (int)ctrl_state;

      /* ================================================================ */
      /* 状态机                                                            */
      /* ================================================================ */

      if (ctrl_state == STATE_RTZ)
      {
        /* ---- STATE_RTZ：缓慢归零 --------------------------------------
         *
         *  Pitch（MOTOR4）：IMU pitch 闭环，目标 0°（真实水平）
         *  Yaw  （MOTOR2）：IMU yaw   闭环，目标 0°（上电正前方）
         *
         *  缓慢由外环输出限幅 RTZ_*_SPD_MAX 保证，PID 计算出的速度目标
         *  被钳位在此范围内，内环（速度环）再跟随，运动自然缓慢平滑。
         *  调小 RTZ_PITCH_SPD_MAX / RTZ_YAW_SPD_MAX 可进一步降低速度。
         *
         *  两轴 IMU 误差均小于门限后：
         *    1. 清空两轴 PID 积分
         *    2. 重新记录编码器零点（使扫描段基准与归零位置对齐）
         *    3. 切换到 STATE_HOMING
         * -------------------------------------------------------------- */

        /* ---- Pitch 外环：IMU pitch 闭环，目标 0° ---- */
        dbg_err4          = angle_err_calc(0.0f, dbg_pitch);
        ...
        float rtz_spd4    = angle_pid4.p_out + angle_pid4.i_out + angle_pid4.d_out;
        LIMIT_MIN_MAX(rtz_spd4, -RTZ_PITCH_SPD_MAX, RTZ_PITCH_SPD_MAX);  /* 限速 → 缓慢 */

        /* ---- Pitch 内环：速度环 ---- */
        {
          float spd_err4    = rtz_spd4 - filtered_spd4;
        ...
        /* ---- Yaw 外环：IMU yaw 闭环，目标 0°（上电正前方）---- */
        dbg_err2          = angle_err_calc(0.0f, yaw_m);
        ...
        float rtz_spd2    = angle_pid2.p_out + angle_pid2.i_out + angle_pid2.d_out;
        LIMIT_MIN_MAX(rtz_spd2, -RTZ_YAW_SPD_MAX, RTZ_YAW_SPD_MAX);     /* 限速 → 缓慢 */

        /* ---- Yaw 内环：速度环 ---- */
        {
          float spd_err2    = rtz_spd2 - filtered_spd2;
        ...
        /* ---- 到位检测：两轴 IMU 误差均小于门限则切状态 ---- */
        uint8_t pitch_ok = (fabsf(dbg_pitch) < RTZ_PITCH_THRESH);
        uint8_t yaw_ok   = (fabsf(yaw_m)     < RTZ_YAW_THRESH);

        if (pitch_ok && yaw_ok)
        {
          /* 清空两轴 PID 积分，防止残余积分在后续段引起冲击 */
          angle_pid2.i_out = 0.0f; speed_pid2.i_out = 0.0f;
          angle_pid4.i_out = 0.0f; speed_pid4.i_out = 0.0f;
          /* 归零到位后重新记录编码器零点，使扫描段基准与当前位置对齐 */
          zero_enc2  = motor_info[MOTOR2_IDX].rotor_angle;
          zero_enc4  = motor_info[MOTOR4_IDX].rotor_angle;
          prof_pos   = 0.0f;
          scan_phase = 0.0f;
          ctrl_state = STATE_HOMING;
        }
      }
      else if (ctrl_state == STATE_HOMING)
      {
        /* ---- STATE_HOMING：归位 ----------------------------------------
         *
         *  归零完成后编码器已重新归零，prof_pos=0，两轴编码器误差理论为 0，
         *  此段仅做稳定性过渡，确认误差后立即切换到 STATE_SCAN。
         * -------------------------------------------------------------- */
        ...
        uint8_t pitch_homed = (fabsf(angle_err_calc(0.0f, dbg_angle4)) < HOMING_THRESH);
        uint8_t yaw_homed   = (fabsf(angle_err_calc(0.0f, dbg_angle2)) < HOMING_THRESH);

        if (pitch_homed && yaw_homed)
          ctrl_state = STATE_SCAN;
