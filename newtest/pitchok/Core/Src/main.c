    ...
    if (HAL_GetTick() - last_tick >= 10)   /* 10ms = 100Hz，与 MahonyAHRS.c sampleFreq 一致 */
    {
      ...
      if (!imu_calib_done)
      {
        ...
        if (calib_cnt >= calib_total)
        {
           ...
           *
           *  原来用 Mahony 500 次收敛来记录安装偏移，存在两个问题：
           *    1. 喂入的是最后一帧陀螺值（非零），收敛位置有偏差
           *    2. 此时 AKF 尚未初始化，陀螺未经滤波
           *
           *  改为直接用重力方向计算 roll/pitch，精度更高，
           *  上电静止时输出严格归零。
          ...
          /* 轴映射（accel X/Y 交换，与正常运行段一致）*/
          float ax0 = ay_mean;   /* Mahony ax ← BMI088 accelY */
          float ay0 = ax_mean;   /* Mahony ay ← BMI088 accelX */
          float az0 = az_mean;

          /* 直接由重力方向计算 roll/pitch 安装偏移 */
          euler_offset[0] = atan2f(ay0, az0) * rad_to_angle;   /* roll  */
          float sin_p0 = -ax0;
          if      (sin_p0 >  1.0f) sin_p0 =  1.0f;
          else if (sin_p0 < -1.0f) sin_p0 = -1.0f;
          euler_offset[1] = asinf(sin_p0) * rad_to_angle;       /* pitch */
      ...
      float gz = gz_akf - gyro_bias[2];   /* Mahony gz ← BMI088 gyroZ（AKF后） */
      ...
      {
        ax = accel[1];   /* Mahony ax ← BMI088 accelY（pitch 轴重力分量）*/
        ay = accel[0];   /* Mahony ay ← BMI088 accelX */
        az = accel[2];
      }
      else
      {
      ...
      #define YAW_ZUPT_KP     0.5f    /* 加大比例增益，让零偏估计快速收敛 */
      #define YAW_ZUPT_KI     0.05f  /* 积分增益略微加大，消除长期偏置   */
