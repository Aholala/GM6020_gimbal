...
#define ACCEL_VALID_MIN   3.0f  /* 约 0.31g，放宽下限 */
#define ACCEL_VALID_MAX  25.0f  /* 约 2.55g，放宽上限容纳扫描振动 */
...
/* 校准结束时的姿态快照（固定目标，不随电机运动变化） */
float align_target_pitch = 0.0f;
float align_target_yaw   = 0.0f;
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_tick >= 1)   /* 10ms = 100Hz，与 MahonyAHRS.c sampleFreq 一致 */
    {
      ...
      if (!imu_calib_done)
      {
        ...
        /* Mahony 同步运行：twoKp=40 极大，加速度计主导，约 500ms 收敛到真实姿态
         * 校准结束时 dbg_pitch_mahony 已是真实物理角度，快照值可靠             */
        MahonyAHRSupdateIMU(q, gyro[0], gyro[1], gyro[2],
        ...
        if (calib_cnt >= calib_total)
        {
          ...
          imu_calib_done = 1;
          /* 校准结束时用加速度计算出的安装偏置作为对齐目标（可靠）*/
          align_target_pitch = imu_pitch_offset;
          align_target_yaw   = dbg_yaw;
...
#define YAW_BIAS_MAX    0.1f    /* 限制最大补偿值 */
      ...
      /* Mahony 原始输出挂 Ozone（物理绝对角度，用于对比）*/
      ...
      /* ---------------------------------------------------------------- */
      /* 4a. TOP_IMU_ALIGN：把云台转到物理水平位置（安装偏置=0）          */
      /*                                                                   */
      /*  目标：让 dbg_pitch_mahony → 0，dbg_yaw → 0                     */
      /*  即驱动云台转到使 IMU 输出为水平的位置，消除安装偏置。            */
      /*  使用未减 offset 的原始 Mahony 输出作为反馈，                     */
      /*  dbg_pitch_offset / dbg_roll_offset 仍可在 Ozone 观测。          */
      /* ---------------------------------------------------------------- */
      if (top_state == TOP_IMU_ALIGN)
      {
        ...
        /* ---- 梯形规划：prof_yaw 逼近 -align_target_yaw ---- */
        {
          float real_target2 = -align_target_yaw;
        ...
        /* ---- 梯形规划：prof_pitch 逼近 -align_target_pitch（物理水平）---- */
        /* 用校准结束时的快照作为固定目标，避免电机运动时目标随 IMU 漂移    */
        {
          float real_target4 = -align_target_pitch;
        ...
        /* 两轴到位判断：编码器已转到抵消安装偏置的位置 */
        if (fabsf(angle_err_calc(-align_target_pitch, dbg_angle4)) < HOMING_THRESH &&
            fabsf(angle_err_calc(-align_target_yaw,   dbg_angle2)) < HOMING_THRESH)
        {
          /* 记录此时 IMU 输出作为零点，后续 dbg_pitch/yaw 减掉此偏置 */
          imu_zero_pitch = pitch_m;
          imu_zero_yaw   = 0.0f;   /* yaw 无绝对参考，以编码器零点为基准 */
          ...
          scan_phase = 0.0f;
          dbg_spd_ff = 0.0f;
          /* 直接进入扫描，跳过 encoder 归位阶段（当前位置即为零点）*/
          state2     = STATE_SCAN;
          state4     = STATE_SCAN;
