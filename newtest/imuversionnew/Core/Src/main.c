...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_tick >= 10)   // 10ms 周期 = 100Hz，与 MahonyAHRS.c 的 sampleFreq 一致
    {
      ...
      /* DMA 未就绪时沿用上一帧 gyro/accel，Mahony 照常推进，不引入额外延迟 */
      ...
      else
      {
        ...
        // ---- 四元数 → 欧拉角（ZYX，单位：度，范围 -180 ~ +180） ----
        {
          ...
          // 规范化到 [-180, +180)
          // 注意：pitch 的物理范围本身是 [-90, +90]，不做翻折处理
