...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_pid_tick >= 10)
    {
      ...
      dbg_target_speed1 = pid_calc(&angle_pid1, dbg_angle_error1, 0.0f);

      // ----- 4. 内环：目标速度 → 电压 -----
      dbg_voltage1 = pid_calc(&speed_pid1, dbg_target_speed1, dbg_current_speed1);
      ...
      dbg_target_speed2 = pid_calc(&angle_pid2, dbg_angle_error2, 0.0f);

      // ----- 4. 内环：目标速度 → 电压 -----
      dbg_voltage2 = pid_calc(&speed_pid2, dbg_target_speed2, dbg_current_speed2);
