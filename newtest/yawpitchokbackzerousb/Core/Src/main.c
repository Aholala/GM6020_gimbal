 ...
 *  STATE_TRACK     自瞄跟随（视觉绝对角度直接作为目标位置，带限位保护）
 *
 * ================================================================= */
typedef enum {
  STATE_HOMING = 0,
...
/* 自瞄目标角度（°），视觉下发的世界系绝对角，经限位 clamp 后直接作为 PID 目标 */
float track_target_yaw   = 0.0f;  /* ID2 yaw 目标（编码器绝对角，°）*/
float track_target_pitch = 0.0f;  /* ID4 pitch 目标（编码器绝对角，°）*/
 ...
 *    dbg_vis_yaw                  视觉下发 yaw（度，世界系绝对角）
 *    dbg_vis_pitch                视觉下发 pitch（度，世界系绝对角）
 *
...
volatile float   dbg_vis_yaw      = 0.0f;   /* 视觉下发 yaw（度，世界系绝对角）*/
volatile float   dbg_vis_pitch    = 0.0f;   /* 视觉下发 pitch（度，世界系绝对角）*/
...
#define YAW_BIAS_MAX    0.05f
       ...
       *
       *   自瞄逻辑说明：
       *     视觉下发的 yaw_rad/pitch_rad 字段（字段名沿用，单位实为度）
       *     是视觉节点在世界坐标系中解算出的目标绝对角度，
       *     与 MCU 上报的 curr_yaw（编码器）共用同一坐标系，
       *     因此可以直接作为 PID 位置环目标，无需再做增量叠加。
       *
       *     进入 STATE_TRACK 时，track_target 先初始化为当前编码器位置，
       *     后续每帧用视觉下发的绝对角刷新，施加限位保护后送 PID。
       *
       *   切换规则（优先级从高到低）：
       *     1. 任意状态：detected=1 且归位完成 → STATE_TRACK
       *     2. STATE_TRACK：detected=0 超过 LOST_TIMEOUT_MS
       *                     → 清积分 → STATE_RETURNING
      ...
      /* ---- 4a-2. 视觉帧活性检测 ----------------------------------------
       *
       *  只有明确收到 detected=0 的帧才认为丢失目标。
       *  无新帧时沿用上一帧的 detected 值，避免视觉帧率低于控制频率时
       *  误触发丢目标切换（视觉帧率通常低于 100Hz 控制周期）。
       *    - 有新帧且 detected=1：vision_active=1，保持自瞄
       *    - 有新帧且 detected=0：vision_active=0，触发切换
       *    - 无新帧：vision_active 沿用上一帧结果，不做改变
       * ------------------------------------------------------------------- */
      static uint32_t last_vis_rx_ok = 0;
      static uint8_t  vision_active  = 0;   /* 静态：无新帧时保持上一帧结果 */
      ...
      /* vis_rx_ok 没变：无新帧，vision_active 保持上一帧值不变 */

      /* ---- 4a-3. 状态切换与目标角更新 -----------------------------------
       *
       *  vision_active=1（有帧且 detected=1）：
       *    首次进入 STATE_TRACK → 以当前编码器位置初始化 track_target，
       *                           清积分，防止首帧突跳
       *    已在 STATE_TRACK     → 直接用视觉下发的绝对角刷新 track_target，
       *                           施加限位后送 PID（不做增量叠加）
       *
       *  vision_active=0（无帧或 detected=0）：
       *    处于 STATE_TRACK     → 开始计时；超过 LOST_TIMEOUT_MS 后：
       *                           清两轴 PID 积分 → 切 STATE_RETURNING
       * ------------------------------------------------------------------- */
      if (homing_done && vision_active)
      {
        ...
        {
          /* ----------------------------------------------------------------
           * 已在 STATE_TRACK：用视觉下发的世界系绝对角直接刷新目标
           *
           *   视觉下发字段（字段名沿用旧协议）：
           *     g_vision_rx.yaw_rad   → 目标 yaw  绝对角（°）
           *     g_vision_rx.pitch_rad → 目标 pitch 绝对角（°）
           *
           *   视觉节点（processor_node.cpp）将相机解算结果转换到世界系，
           *   并用 MCU 上报的 curr_yaw（编码器角）作为参考，
           *   因此下发的绝对角与编码器零点一致，可以直接作为 PID 目标。
           * ---------------------------------------------------------------- */
          float vis_yaw_abs   = g_vision_rx.yaw_rad;   /* 视觉目标 yaw  绝对角（°）*/
          float vis_pitch_abs = g_vision_rx.pitch_rad; /* 视觉目标 pitch 绝对角（°）*/

          /* 施加限位 clamp，防止越界 */
          if      (vis_yaw_abs >  TRACK_LIMIT_YAW)   vis_yaw_abs =  TRACK_LIMIT_YAW;
          else if (vis_yaw_abs < -TRACK_LIMIT_YAW)   vis_yaw_abs = -TRACK_LIMIT_YAW;
          if      (vis_pitch_abs >  TRACK_LIMIT_PITCH) vis_pitch_abs =  TRACK_LIMIT_PITCH;
          else if (vis_pitch_abs < -TRACK_LIMIT_PITCH) vis_pitch_abs = -TRACK_LIMIT_PITCH;

          track_target_yaw   = vis_yaw_abs;
          track_target_pitch = vis_pitch_abs;
        }
      }
      ...
      /*    pitch_deg：IMU Mahony 解算 pitch（度），视觉节点用于坐标变换    */
      /*    yaw_deg  ：编码器 yaw 角（度），即 dbg_yaw_enc                 */
      /*               视觉节点将此值作为世界系 yaw 参考，解算绝对目标角    */
      /*    mode     ：0x01=扫描/回零/归位  0x02=自瞄                     */
      /* ---------------------------------------------------------------- */
      {
        uint8_t tx_mode = (state2 == STATE_TRACK) ? 0x02 : 0x01;
        BSP_USB_Send(dbg_pitch, dbg_yaw_enc, tx_mode);  /* pitch 用 PF 输出，yaw 用编码器 */
      }
